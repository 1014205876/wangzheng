...
export class ProductInfoComponent implements OnInit {
  ...
  breadcrumbs = ['业务管理', '产品发布管理', '产品详细信息'];
  ...
  ngOnInit() {}
  // /peak-resource/v1/resource/product/extend/267c12f4-cc00-4f7d-a633-0b980f5fa253
  getinfor(id) {//获取产品信息
    this.http.get(
      'peak-resource/v1/resource/product/extend/' + id
    ).then(res => {
      this.data = res.data;
      this.foreach();
    })
  ...
  foreach() {
    this.repayMethodName.length = 0;
    this.productTypeName = '';
    this.interestStartTimeName = '';
    this.paymentMethodName = '';
    for (let i = 0; i < this.data.product.repayMethodShow.length; i++) {
      if (this.data.product.repayMethodShow[i].checked == 1) {
        this.repayMethodName.push(this.data.product.repayMethodShow[i].name)
      }
    }
    for (let i = 0; i < this.data.product.productTypeShow.length; i++) {
      if (this.data.product.productTypeShow[i].checked == 1) {
        this.productTypeName = this.data.product.productTypeShow[i].name
      }
    }
    for (let i = 0; i < this.data.product.interestStartTimeShow.length; i++) {
      if (this.data.product.interestStartTimeShow[i].checked == 1) {
        this.interestStartTimeName = this.data.product.interestStartTimeShow[i].name
      }
    }
    for (let i = 0; i < this.data.product.paymentMethodShow.length; i++) {
      if (this.data.product.paymentMethodShow[i].checked == 1) {
        this.paymentMethodName = this.data.product.paymentMethodShow[i].name
      }
    }
  }
