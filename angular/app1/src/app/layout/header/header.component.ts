...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    /** 其他巅峰平台链接跳转 */
    this.http.getApiNull("/api/peak-resource/v1/resource/apps/permission").then(data => {
      this.nav = data;
    }, (error) => {
      this.message.create('error', error, { nzDuration: 3000 });
    })
    /** 登录状态信息 */
    this.http.getApiNull("/api/peak-resource/v1/resource/user/info").then(data => {
      this.userInfor = data;
    }, (error) => {
      this.message.create('error', error, { nzDuration: 3000 });
    })
  ...
  choiceOk(): void {
    ...
    if (this.resetPwd.valid) {
      ...
      }, (error) => { this.message.create('error', error, { nzDuration: 5000 }) })
    }
  }
