...
export class ModifyComponent implements OnInit {
	...
	constructor(private http: HttpServe, private route: ActivatedRoute, private toastyService: ToastyService) {
	...
	user() {
		this.http.getCustomHeaders('v1/resource/user/' + this.id).subscribe((res) => {
			var data = res.result
	...
	onSubmit() {
		...
		this.http.putCustomHeaders('v1/resource/user/' + this.id, this.items).subscribe((res) => {
			var code = res.code
			if (code == '200') {
				this.goBack()
			} else {
	...
	getDepartment() {
		this.http.getCustomHeaders('v1/resource/department').subscribe((res) => {
