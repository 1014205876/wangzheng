...
export class DictionaryComponent implements OnInit {
  ...
  async search(e) {
    ...
    let data = await this.api.getDictionaryList(params)
    this.rows = []
    this.rows = data.list;
  ...
  async submited(e) {
    ...
    // debugger
    // console.log(this.addForm,this.modalItems)
    if (this.modalStatus == 'edit') {
      let res = await this.api.editDictionary(this.addForm.value)
      if (res) {
      ...
      let res = await this.api.addDictionary(this.addForm.value)
      if (res) {
        this.message.create('success', '成功!', { nzDuration: 3000 });
        this.modalCancel();
        this.resetForm(e);
