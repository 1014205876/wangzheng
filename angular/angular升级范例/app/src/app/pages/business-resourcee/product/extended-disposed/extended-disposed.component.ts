...
@Component({
  ...
  templateUrl: './extended-disposed.component.html',
  styleUrls: ['./extended-disposed.component.less']
})
export class ExtendedDisposedComponent implements OnInit {
  /** 列表title */
  title = "";
  id = "";
  // 弹窗展示字段
  ...
  async search(e) {
    ...
    let res = await this.api.getSysProductExtendSetting(this.id, params)
    this.rows = [];
    this.rows = res.list;
  ...
  submited(e) {
    ...
    let data = {
      ...
      "type": (this.addForm.value.type ? this.addForm.value.type : ''),
  ...
  // 添加扩展配置
  async add(data) {
    let res = await this.api.addSysProductExtendSetting(data)
    this.loading = false;
    if (res) {
      this.show = false;
  ...
  //修改配置项
  async edit(data) {
    let res = await this.api.editSysProductExtendSetting(this.rowId, data)
    this.loading = false;
    if (res) {
      this.show = false;
  ...
  //删除配置项
  async delete(data) {
    let res = await this.api.deleteSysProductExtendSetting(data.id)
    if (res) {
